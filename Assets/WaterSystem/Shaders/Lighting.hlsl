...
{
    float f = saturate(pow(1.0 - dot(normalWS, viewDirectionWS), 5))/* * saturate(pow(1.0 - dot(float3(0, 1, 0), viewDirectionWS), 1.0))*/;
    return f;
}
...
half3 SampleReflections(bool isUnderwater, half3 normalWS, half3 viewDirectionWS, half2 screenUV, half roughness)
{
	...
	half2 reflectionUV = screenUV + normalWS.zx * half2(0.02, 0.15) * 2;// * length(viewDir) * 1;
	...
	half3 planarReflection = SamplePlanarReflections(reflectionUV).rgb;
	//float t = lerp(0.9, 1, dot(normalWS, viewDirectionWS));
